<template>
    <div id="admin_borrowedList">
        <el-button-group style="margin-bottom:10px">
            <el-button icon="el-icon-upload2" size="small" title="导出借书列表" @click.native="exportList"></el-button>
        </el-button-group>
        <a v-if="exportURL" :href="exportURL" target="_blank" @click="download">{{exported?'重新下载':'点击下载'}}</a>
        <el-table
        :data="books"
        border
        style="width: 100%">
            ...
            <el-table-column
            label="借出信息(借书人/书籍编号/借出时间)">
              <ul style="list-style: none;padding: 0;" slot-scope="{row}">
...
<script>
  ...
  export default {
    ...
    methods: {
      search(){
        Vue.fetch({method:'/book/getBorrowedBooks',type: 'get',data:{pageSize: this.pageSize, page: this.page}}).then(res=>{
      ...
      async exportList() {
        const res = await Vue.fetch({method:'/book/export',type: 'get'});
        if (res) {
          this.exportURL = res;
          this.exported = false;
        }
