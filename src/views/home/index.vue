<template>
  <div id="home_index">
    <div v-if="stitle" style="font-size: 14px; margin-bottom: 8px; color: #aaa">
      搜索：`{{ stitle }}`
    </div>
    <el-row :gutter="20">
      <el-col
  ...
  methods: {
    ...
    async borrow(index) {
      ...
      const identifier = await fetch({
        method: '/user/borrow',
        data: { id: this.books[index]._id },
      })
      if (identifier) {
        this.$message.success(`去书架上领取吧，编号：${identifier}！`)
        this.userInfo.borrowedBooks.push({
          id: this.books[index]._id,
          identifier,
        })
        this.books[index].borrowers.push({
